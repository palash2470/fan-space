        ...
        <?php
          foreach ($meta_data['orders'] as $key => $value) {
            ...
            echo '<tr order_status="' . $value->status . '">
                ...
                <td><a href="javascript:;" class="commonBtn2 toggleRowDetails">View</a> <a href="javascript:;" class="commonBtn2 edit_order" order_id="' . $value->id . '">Status</a></td>
            </tr>
            <tr class="rowDetailsRow"><td colspan="7" style="display: none;">
              ...
              <table class="table table-hover">
                <thead><tr><th scope="col">Name</th><th scope="col">Description</th><th scope="col">view</th><th scope="col">Coins</th><th scope="col">Quantity</th><th scope="col">Total Amount</th></tr></thead>
                <tbody>';
                foreach ($value->item_data as $k => $v) {
                  echo '<tr><td><a href="' . url('/dashboard/product?id=' . $v->product_id) . '" target="_blank">' . $v->title . '</a></td><td>'.$v->description.'</td><td>' .($v->attachment != '' ? '<button type="button" class="btn btn-primary myModal" data-toggle="modal" data-attachmentType="'.$v->type.'" data-attachment="'.$v->attachment.'" data-target="#myModal"><i class="fas fa-eye"></i></button>': ''). '</td><td>' . $v->price . '</td><td>' . $v->quantity . '</td><td>' . ($v->price * $v->quantity) . '</td></tr>';
                }
                echo '</tbody>
              </table>
            </td></tr>
...
@include('user.includes.vip_member._partials.modal_order_edit')
    ...
    $(document).on('click', '.edit_order', function(){
      ...
      $.ajax({type: 'POST', dataType: 'json', url: prop.ajaxurl, data: data, processData: false, contentType: false, success: function(data){
          ...
          if(data.success == '1') {
            ...
            $('#orderEditModal select[name="status"]').val(order.status);
            $('#orderEditModal input[name="order_id"]').val(order.id);
            $("#orderEditModal").modal({backdrop: 'static', keyboard: false});
          }
        }
      });
    });

    $(document).on('click', '#orderEditModal .orderEditModal_submit', function(){
      var status = $('#orderEditModal select[name="status"]').val();
      var order_id = $('#orderEditModal input[name="order_id"]').val();
      $(".mw_loader").show();
      var data = new FormData();
