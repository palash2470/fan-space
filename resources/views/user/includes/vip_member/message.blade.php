    ...
    <div class="messages_display_box">
      ...
      <div class="for-mobile-chat"><button type="button" class="chat-open-btn"><i class="fas fa-bars"></i></button></div>
        <div class="d-flex align-items-center justify-content-between lft-gap">
            <div class="d-flex align-items-center messages_profile_display_box"> <span class="contact-profile-img"><img src="<?php if(isset($meta_data['friend_list'][0]['user_img'])){ echo $meta_data['friend_list'][0]['user_img'];} ?>" alt="" /></span>
            <p>
                <?php if(isset($meta_data['friend_list'][0]['name_name'])){ echo $meta_data['friend_list'][0]['name_name'];} ?>
            </p>
            </div>
...
/* function get_friend_chat(friend_id) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; data.result.length > i; i++) {
                if (data.result[i].is_me == 'Y') {
                    html += '<div class="outgoing-msg d-flex justify-content-end mb-2"><div class="sent-msg"><p>' + data.result[i].msg + '</p><span class="time-date"> ' + data.result[i].time + '</span></div><div class="user-img-wrap"><span class="user-img"> <img src="' + data.result[i].profile_photo + '" alt=""></span></div></div>';
                } else {
                    html += '<div class="incoming-msg d-flex justify-content-start mb-2"><div class="user-img-wrap"><span class="user-img"><img src="' + data.result[i].profile_photo + '" alt=""></span></div><div class="received-msg"><p>' + data.result[i].msg + '</p><span class="time-date"> ' + data.result[i].time + '</span></div></div>';
                }
            }
			$('#chat_box').html(html);
			$('.msg-history-scrollbar').scrollTop($('.msg-history-scrollbar')[0].scrollHeight);
			$('.messages_profile_display_box').html('<span class="contact-profile-img"><img src="'+data.friend_data.user_img+'" alt=""></span><p>'+data.friend_data.user_title+'</p>');
