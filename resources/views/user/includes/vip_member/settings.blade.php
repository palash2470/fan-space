<?php
// dd(\App\User::find(9)->user_meta_array()['thank_you_msg']);
// $meta = \App\User::find(5)->user_meta;
// $usermeta_data = [];
//                 foreach ($meta as $k => $v) {
//                     $usermeta_data[$v->key] = $v->value;
//                 }
//             dd($usermeta_data);
// $usermeta = new \App\User_meta;
// dd($usermeta->get_usermeta(9));
...
<div class="settings_form"><!--settings_form-->
  ...
  <div class="from-wrap-page">
    ...
    <div class="form-group from-input-wrap relative phone-box">
      ...
      <input type="tel" name="mobile" id="input-phone" class="input-3" placeholder="Phone Number" value="{{ $user_data['phone'] }}" readonly="readonly" />
    ...
    <!--<div class="form-group from-input-wrap relative">
      <input type="hidden" name="phone_otp_verify" id="phone_otp_verify" value="true" />
      <label for="">Phone Number</label>
      <input type="text" name="mobile" id="input-phone" class="input-3" placeholder="Phone Number" value="{{ $user_data['phone'] }}" readonly />
      <div class="vfWrap verifybtn" style="display:none"><a href="javascript:;" class="vfBtn sendPhoneOtp"><span>Confirm text to</span>Verify</a></div>
      <div class="vfWrap verifybtn"><a href="javascript:;" class="vfBtn editPhone"> <i class="fas fa-edit"></i> Edit</a></div>
      <span class="errMsg" id="phone_otp_error" style="display:none">Error</span> </div>-->
    <!--<div class="form-group from-input-wrap relative">
  <input type="hidden" name="email_otp_verify" id="email_otp_verify" value="false" />
      <label for="">Phone Number</label>
      <input type="text" name="mobile" id="input-email" class="input-3" placeholder="Phone Number" value="{{ $user_data['phone'] }}" />
    ...
    <div class="form-group from-input-wrap mt-3">
      ...
      <div class="file-upload active" allowedExt="jpg,png" maxSize="4194304" maxSize_txt="4mb" no_preview>
        <div class="file-preview--- relative text-left"> <a href="{{ url('public/uploads/id_proof_doc/' . $user_data['meta_data']['id_proof_doc']) }}" target="_blank">View / Download</a> </div>
    ...
    <div class="form-group from-input-wrap mt-3">
      ...
      <div class="file-upload active" allowedExt="jpg,png" maxSize="4194304" maxSize_txt="4mb" no_preview>
        <div class="file-preview--- relative text-left"> <a href="{{ url('public/uploads/id_proof/' . $user_data['meta_data']['id_proof']) }}" target="_blank">View / Download</a> </div>
        <div class="file-select">
          <div class="file-select-button" id111="fileName">Choose File</div>
    ...
    ?>
      <div class="file-upload {{ $profile_video != '' ? 'active' : '' }}" allowedExt="mp4" maxSize="20971520" maxSize_txt="20mb" no_preview>
        <div class="file-preview--- relative text-left"> <a href="{{ url('public/uploads/profile_video/' . $profile_video) }}" target="_blank">View / Download</a> </div>
        <div class="file-select">
          <div class="file-select-button" id111="fileName">Choose File</div>
    ...
    ?>
      <div class="file-upload {{ $profile_banner != '' ? 'active' : '' }}" allowedExt="jpg,png" maxSize="10485760" maxSize_txt="10mb" preview>
        <div class="file-preview"> <img src="{{ url('public/uploads/profile_banner/' . $profile_banner) }}" class="sel_img" /> </div>
        <div class="file-select">
          <div class="file-select-button" id111="fileName">Choose File</div>
  ...
  ?>
    ...
    <div class="form-group from-input-wrap mt-3" style="display:none">
    ...
    <div class="form-group from-input-wrap mt-3">
      ...
      ?>
      <input type="text" name="location" id="" class="form-control input-3" google_location_search_callback="vipmember_settings_google_location_search" value="{{ $location }}">
    ...
    <div class="form-group from-input-wrap">
      <label for="">Automatic thanks you message to the new subscriber</label>
    ...
    <div class="form-group from-input-wrap">
      ...
      <div class="d-flex align-items-center">
        <p>{{ url('/affiliate/' . $user_data['username']) }}</p>
        <button type="button" class="copy-btn btn-copy js-tooltip js-copy" data-toggle="tooltip" data-placement="bottom" title="Copy to clipboard" data-copy="{{ url('/affiliate/' . $user_data['username']) }}"><i class="fas fa-clipboard-list" ></i></button>
      </div>
    </div>
    <button type="button" class="commonBtn settings_submit">Update</button>
  </div>
</div>
@push('script')
    ...
    if (otp_password == '') {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.action == 1) {
					$('.settings_form input[name="mobile"]').closest('.form-group').find('.error').hide();
...
$(document).on('click', '.sendEmailOtp', function() {
    ...
    if (email=='') {
        ...
        $.ajax({
            ...
            success: function(data) {
				$('.settings_form input[name="email"]').closest('.form-group').find('.error').hide();
                if (data.success == 0) {
					$('.settings_form input[name="email"]').closest('.form-group').append("<div class='error'>"+data.error_msg+"</div>");
                    $('#input-email').removeClass('black_border').addClass('red_border');
                } else {
...
function matchEmailOtpPassword() {
    ...
    function vipmember_settings_google_location_search() {
        autocomplete_vip = new google.maps.places.Autocomplete($('.settings_form input[name="location"]')[0], {
        ...
        autocomplete_vip.addListener("place_changed", function(){
            ...
            console.log(zipcode);*/
            $('.settings_form input[name="address_line_1"]').val(address_line_1.join(', '));
            $('.settings_form input[name="country_code"]').val(country_code);
            $('.settings_form input[name="zip_code"]').val(zipcode);
        });
    }
