<div class="row">
  <?php
  foreach ($meta_data['following'] as $key => $value) {
    ...
    <div class="col-lg-3 col-md-4 col-sm-6 col-12 m-b-20">
        <div class="performerBox w-100 active wow wow fadeIn delay1 relative" style="visibility: visible; animation-name: fadeIn;">
            ...
            <div class="performerdecc w-100 text-center">
                <h4><a href="{{ url('u/' . $value->username) }}">{{ $value->display_name }}</a><!-- <span><i class="ti-heart"></i></span> --></h4>
                <h5>{{ '@' . $value->username }}</h5>
            </div>
            <div class="performerBtn text-center">
                <a href="javascript:;" class="set_user_follow" user_id="{{ $value->id }}"><i class="fas fa-thumbs-down"></i> Unfollow</a>
            </div>
        </div>
    </div>
    <?php
